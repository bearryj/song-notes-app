...
- [x] Virtual scrolling for song list — with hundreds of songs, rendering all DOM nodes is slow. Virtualize the list to only render visible items. (2026-06-13) — buildVirtualItems() creates flat array of headers+songs with pixel offsets; renderVirtualItems() uses binary search to find visible range, renders only items in viewport + 8-item buffer; scroll listener with rAF throttling; swipe actions preserved via absolute positioning
