...
## Last Updated
2026-06-16 by OWL (feat: add PWA manifest and service worker for offline support)
...
- Instrument tuning presets for tuner (6 presets: Guitar Standard, Drop D, Open G, DADGAD, Ukulele GCEA, 4-String Bass; preset selector row above string buttons; switches string notes/frequencies/target; rebuilt string buttons per preset with haptic feedback; light theme support; ARIA roles) (2026-06-15)
...
- Bulk tag operations in multi-select mode — new # button in the multi-select bar opens a bottom sheet showing all existing tags as tappable chips. Tags on all selected songs show ✓ (all), tags on some show • (partial), tags on none show + (tap to add). Create new tags via the input field. Toggling a tag that is on all selected songs removes it from all; tapping a tag on none (or some) adds it to all. State refreshes live, and the song list and tag filter bar update immediately. (2026-06-15)
- PWA manifest and service worker — adds manifest.json (app name, icons, theme colors, standalone display) and sw.js (cache-first for static assets, network-first for API calls). Enables home screen installation and offline caching for web preview. Fixes missing files referenced in index.html since initial PWA setup. (2026-06-16)
